...
import EditarColumna from "./EditarColumna";
...
const SpaceWork = ({ projectId }) => {
  ...
  const AbrimodoEdit = (columnId) => {
    setEditMode(true);
    setEditingColumnId(columnId); // Establece el ID de la columna en edición
    console.log("EditMode is now", EditMode);
  };

  const CerrarModoEdit = () => {
    setEditMode(false);
    setEditingColumnId(null); // Limpia el ID de la columna en edición
  };
                            ...
                            {openMenuId === column ? (
                              <div className="flex h-60 justify-center items-center">
                                <div className="flex flex-col items-center justify-center space-y-2">
                                  <button
                                    onClick={() => AbrimodoEdit(column.id)}
                                  >
                                    <EditarColumna
                                      column={column.id}
                                      projectId={projectId}
